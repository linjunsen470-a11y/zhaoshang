...
import { mapProject } from '../api/_shared/payloadApi'
...
const serviceCards = [
  ...
  {
    title: '品牌合作入口',
    desc: '品牌方找校园点位、学校或物业引入品牌，保留轻量表单和顾问对接。',
    stat: '轻量拓展',
  },
]
...
async function getOpportunities() {
  try {
    ...
    return result.docs.map(doc => mapProject(doc) as Opportunity)
  } catch (dbErr) {
    ...
    <main className="min-h-screen bg-[#f7f8fb] text-slate-950">
        ...
        <div className="mx-auto grid max-w-6xl gap-8 px-5 py-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-base leading-8 text-slate-600">
              小程序负责微信内快速咨询和复访，网站负责公开展示、搜索获客和合作方背书。所有需求进入同一个后台，由招商顾问跟进。
            </p>
      ...
      <section className="mx-auto max-w-6xl px-5 py-12">
        <div className="mb-5 flex items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">与 Payload CMS 后台共用数据源，可扩展为独立列表页和详情页。</p>
          </div>
        </div>
          ...
          {opportunities.map(item => (
            <article className="overflow-hidden rounded-md border border-slate-200 bg-white" key={item.id}>
              <div
                ...
                style={{
                  backgroundImage: `url(${item.coverImage || 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=800&q=80'})`,
                }}
              />
              <div className="p-5">
      ...
      <section className="border-t border-slate-200 bg-white" id="lead-form">
        <div className="mx-auto grid max-w-6xl gap-8 px-5 py-12 md:grid-cols-[0.9fr_1.1fr]">
          <div>
            <h2 className="text-2xl font-bold">提交合作需求</h2>
            <p className="mt-3 leading-7 text-slate-600">
              网站表单直接写入 Payload 线索池，来源标记为 website。顾问可在 CMS 后台统一查看、分配和跟进。
            </p>
          </div>
          <form action={submitWebsiteLead} className="grid gap-3 rounded-md border border-slate-200 bg-slate-50 p-5">
            ...
            <input
              ...
              placeholder="经营品类/设备名称/品牌名称"
            />
            <textarea
              name="remark"
              className="min-h-28 rounded-md border border-slate-300 px-3 py-3 text-sm"
              placeholder="请简要说明需求"
            />
