...
Payload CMS 用于生产方向的数据维护。后台已针对编辑/招商顾问做中文化与流程优化（详见 `docs/payload-admin-ux-audit.md`）。
...
| 角色 | 权限 |
|------|------|
| `admin` | 全权限，含用户管理与删除线索 |
| `advisor` | 线索、商户档案、跟进 |
| `editor` | 项目与媒体维护 |

运营工作台（登录后首屏）展示新线索、待跟进、在线项目等统计，并提供看板入口。
...
业务规则：新建项目默认审核状态 `pending`；审核未通过不可上线。编辑页展示对应的小程序详情路径提示。
...
- `attachments`
- `status`
- `owner`（关联 `users`）/ `nextFollowAt` / `closedAmount` / `lostReason`
- `merchantProfile`（关联商户档案）

列表默认列：称呼、电话、类型、状态、负责人、项目、下次跟进、创建时间。

线索详情提供快捷操作：复制电话、打开关联项目、转为项目、沉淀商户档案。

看板视图：`/admin/collections/leads/kanban`，支持拖拽更新状态。

CRM 字段仅允许在 CMS 或内部管理流程中修改，不允许小程序 API 直接写入或覆盖。

### 4.4 Merchant Profiles

商户长期需求档案，字段含联系方式、经营品类、预算、区域偏好等。

- 线索可通过 `POST /api/leads/:id/sync-merchant` 沉淀为档案。
- 档案详情展示关联线索面板（`relatedLeads`）。
...
- 侧栏隐藏，日常在线索详情「跟进历史」标签页添加。
- 添加跟进时可同步更新线索 `status` 与 `nextFollowAt`。
